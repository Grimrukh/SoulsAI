...
function HeavyKnight_Mace257001Battle_Activate(ai, goal)
    ...
    if myThinkId == 257012 then
        ...
        if not not ai:IsInsideTargetRegion(TARGET_ENE_0, 1502721) or ai:IsInsideTargetRegion(TARGET_ENE_0, 1502722) then
            kisou = 0
        else
            actPerArr[1] = 100
            kisou = 1
...
function HeavyKnight_Mace257001Battle_Interupt(ai, goal)
    ...
    if UseItem_Act(ai, goal, distUseItem_Act, oddsUseItem_Act) then
        ...
        return true
    elseif ai:IsInterupt(INTERUPT_Inside_ObserveArea) and ai:IsFinishTimer(0) == true then
        goal:ClearSubGoal()
        goal:AddSubGoal(GOAL_COMMON_ApproachTarget, 3, POINT_INITIAL, 2, TARGET_ENE_0, true, 9910)
        goal:AddSubGoal(GOAL_COMMON_Wait, 3, TARGET_ENE_0)
        return true
