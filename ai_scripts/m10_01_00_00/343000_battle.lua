...
function Hellkite343000Battle_Activate(ai, goal)
    ...
    if 0 < Approach30Timer then
        if targetDist <= 30 then
            Act12Per = 100
        else
            Act09Per = 100
        end
    elseif 0 < Approach12Timer then
        if targetDist <= 12 then
            Act12Per = 100
        else
            Act09Per = 100
        end
    elseif 0 < ApproachNearestTimer then
    ...
    elseif ai:IsInsideTarget(TARGET_ENE_0, AI_DIR_TYPE_B, 120) and 3 <= targetDist and targetDist <= 16 and fate <= 50 then
        Act08Per = 100 * TailAttack
    elseif (not not ai:IsInsideTargetRegion(TARGET_ENE_0, 1012760) or not not ai:IsInsideTargetRegion(TARGET_ENE_0, 1012761) or not not ai:IsInsideTargetRegion(TARGET_ENE_0, 1012762) or ai:IsInsideTargetRegion(TARGET_ENE_0, 1012763)) and fate_Safety <= 60 then
        local NearestPoint = Find_Nearest_Point(ai, goal)
        ai:SetEventMoveTarget(NearestPoint)
        local Dist_NearestPoint = ai:GetDistAtoB(POINT_EVENT, TARGET_SELF)
        if 3 <= Dist_NearestPoint then
            Act16Per = 100
        else
            Act15Per = 100
        end
    ...
    elseif ai:IsInsideObserve(2) and fate4 <= 40 then
        Act06Per = 100
    elseif ai:IsInsideTarget(TARGET_ENE_0, AI_DIR_TYPE_F, 30) and 6 <= targetDist and targetDist <= 13 and fate5 <= 40 then
        Act07Per = 100
    elseif 40 <= targetDist then
        Act10Per = 100
    elseif 34 <= targetDist then
        if not (not ai:IsInsideTargetRegion(TARGET_SELF, 1012716) or Dist_ENE_1012710 >= Dist_Self_1012710) or ai:IsInsideTargetRegion(TARGET_SELF, 1012717) and Dist_ENE_1012711 < Dist_Self_1012711 then
            Act03Per = 75
        end
        Act10Per = 25
    ...
    elseif 20 <= targetDist then
        ...
        if not (not ai:IsInsideTargetRegion(TARGET_SELF, 1012752) or Dist_ENE_1012710 >= Dist_Self_1012710) or ai:IsInsideTargetRegion(TARGET_SELF, 1012753) and Dist_ENE_1012711 < Dist_Self_1012711 then
            Act14Per = 40
        end
        Act01Per = 14
        Act02Per = 14
    ...
    elseif 8 <= targetDist then
        ...
        if not (not ai:IsInsideTargetRegion(TARGET_SELF, 1012751) or Dist_ENE_1012710 >= Dist_Self_1012710) or ai:IsInsideTargetRegion(TARGET_SELF, 1012750) and Dist_ENE_1012711 < Dist_Self_1012711 then
            Act13Per = 60
        end
        Act06Per = 20
        Act07Per = 20
